...
if __name__ == "__main__":
    ...
    cr_bc_entropy['log10_entropy'] = np.log10(cr_bc_entropy['entropy'] )
    cr_empty_entropy['log10_entropy'] = np.log10(cr_empty_entropy['entropy'] )
    fig, ax = plt.subplots(1, 2, sharey=True)
    ...
    ax[0].set_xlabel('CR cell barcodes')
    ax[1].set_xlabel('CR empty barcodes')
    ax[0].axhline(y=np.log10(entropythreshold ), color='r', linestyle='-', label=f'log10({entropythreshold})')
    ax[1].axhline(y=np.log10(entropythreshold ), color='r', linestyle='-', label=f'log10({entropythreshold})')
    fig.legend()
