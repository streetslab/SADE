...
filtered_bc_notflag_df_file="${output_dir}/entropy_filtered_bc_w_DNAdebrisflag_df.tsv"
...
awk -F',' -v OFS=',' 'NR==1 {print $0}'  "${filtered_bc_notflag_df_file}" > "${filtered_bc_df_file}"   # Write header
awk -F ',' -v OFS=',' '$8=="NO" {print $0}'  "${filtered_bc_notflag_df_file}" >> "${filtered_bc_df_file}"
