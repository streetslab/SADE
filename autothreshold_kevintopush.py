...
def fit_spline_and_find_cutoff(x: np.ndarray, y: np.ndarray, k: int=2,  spline_s: int=67, spline_k: int=3, limit: int=20000) -> tuple[float, int, BSpline]:
    ...
    ** Method:     find the smallest 1st derivative among k inflection regions, return the corresponding rank and entropy cutoff **

    x: np.ndarray, the ranks (1 is highest)
    y: np.ndarray, the log_10 entropy values
    k: int, maximal number of inflection regions to consider. (inflection region is defined as where second derivative goes from negative to positive) 
    ...
    second_deriv = second_deriv / np.abs(second_deriv).max()  # normalize second derivative to better find regions
    ...
    for i in x: 
        ...
        # normalize second derivative to better find regions
        d_2 = second_deriv[i]
...
def get_x_y_from_pickle_helper(pickle_path: str):
    ...
    noNone_entropies = {k: v for k, v in entropies.items() if v is not None}
    sorted_entropies = dict(sorted(noNone_entropies.items(), key=lambda item: item[1], reverse=True))
